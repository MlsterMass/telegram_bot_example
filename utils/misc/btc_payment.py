...
class AddressDetails:
    def __init__(self, address: str, total_received: int,
                 total_sent: int, balance: int,
...
class Payment:
    ...
    def check_payment(self):
        details = blockcypher.get_address_details(address=config.WALLET_BTC, api_key=config.BLOCKCYPHER_TOKEN)
    ...
    def generate_address(self):
        self.address = blockcypher.generate_new_address(api_key=config.BLOCKCYPHER_TOKEN)

    def send_to_myself(self):
        blockcypher.simple_spend(from_privkey=self.address.get('privkey'), to_address=config.WALLET_BTC,
                                 to_satoshis=self.amount, api_key=config.BLOCKCYPHER_TOKEN)